...
mal create_atom(list_node *n, env *e) {
...
mal atom_test(list_node *n, env *e) {
  DEBUG_HIGH_MAL("called with", mal_list(n));
  if (list_count(n) != 1)
    return mal_exception_str("Bad arguments to atom?");
...
mal atom_deref(list_node *n, env *e) {
  DEBUG_HIGH_MAL("called with", mal_list(n));
...
mal atom_reset(list_node *n, env *e) {
  DEBUG_HIGH_MAL("called with", mal_list(n));
...
mal atom_swap(list_node *n, env *e) {
  DEBUG_HIGH_MAL("called with", mal_list(n));
  if (list_count(n) < 2)
...
void add_atom(env *e) {
  env_set(e, "atom", mal_fn(create_atom));
  env_set(e, "atom?", mal_fn(atom_test));
  env_set(e, "deref", mal_fn(atom_deref));
  env_set(e, "reset!", mal_fn(atom_reset));
  env_set(e, "swap!", mal_fn(atom_swap));
}
