...
public class ReaderTests: XCTestCase {
    public func testExpr() throws {
        ...
        XCTAssertEqual(res2, .success(.seq(true, [.int(2)])))
        let (str3, res3) = s(expr(i("'23")))
        XCTAssertEqual(str3, "")
        XCTAssertEqual(res3, .success(.seq(true, [.sym("quote"), .int(23)])))
    ...
    public func testList() throws {
        ...
        XCTAssertEqual(str1, "q")
        XCTAssertEqual(res1, .success(.seq(true, [.int(2), .int(3)])))
        let (str2, res2) = s(list(i("()Q")))
        XCTAssertEqual(str2, "Q")
        XCTAssertEqual(res2, .success(.seq(true, [])))
    ...
    public func testVector() throws {
        ...
        XCTAssertEqual(str1, "q")
        XCTAssertEqual(res1, .success(.seq(false, [.int(2), .int(3), .int(4)])))
        let (str2, res2) = s(vector(i("[]Q")))
        XCTAssertEqual(str2, "Q")
        XCTAssertEqual(res2, .success(.seq(false, [])))
