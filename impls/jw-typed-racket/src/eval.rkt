...
(define (EVAL [eval-ast : Mal] [env : mal-env]) : Mal
  (match (macro-expand eval-ast env)
    ...
    ;; do special form
    [(mal-list _ _ (list 'do))
     (void)]
    [(mal-list _ _ (list 'do args ...))
     (for/last : Mal ([a : Mal args])
       (EVAL a env))]

    ;; fn* special form
    [(mal-list _ _ (list 'fn* (mal-list _ _ binds) ast))
     (mal-function #f #f
      (lambda (calling-params)
        (let ([fn-env (env-new-from-lists binds calling-params env)])
          (EVAL ast fn-env))))]
    [(mal-list h m (list 'fn* (mal-vector #f #f binds) ast))
     (EVAL (mal-list h m (list 'fn* (mal-list #f #f (vector->list binds)) ast)) env)]
    
    ;; if special form
    [(mal-list _ _ (list 'if condition then-ast else-ast))
     (if (mal-truthy? (EVAL condition env))
         (EVAL then-ast env)
         (EVAL else-ast env))]
    [(mal-list h m (list 'if condition then-ast))
     (EVAL (mal-list h m (list 'if condition then-ast (mal-nil))) env)]
    
    ;; let* special form
    [(mal-list _ _ (list 'let* (mal-list _ _ bindings) let-ast))
     (let ([let-env (env-new '() env)])
       (add-bindings! let-env bindings)
       (EVAL let-ast let-env))]
    [(mal-list h m (list 'let* (mal-vector #f #f bindings-vector) let-ast))
     (EVAL (mal-list h m (list 'let* (mal-list #f #f (vector->list bindings-vector)) let-ast)) env)]

    ;; macroexpand special form
    [(mal-list _ _ (list 'macroexpand x))
     (macro-expand x env)]
    
    ;; quasiquote special form
    [(mal-list _ _ (list 'quasiquote x))
     (EVAL (mal-quasiquote x) env)]

    ;; quasiquoteexpand special form
    [(mal-list _ _ (list 'quasiquoteexpand x))
     (mal-quasiquote x)]
    
    ;; quote special form
    [(mal-list _ _ (list 'quote x))
     x]

    ;; try-catch special form
    [(mal-list _ _ (list 'try* a (mal-list _ _ (list 'catch* b c))))
     ...
     (with-handlers
       ...
       (EVAL a env))]
    [(mal-list _ _ (list 'try* single-arg))
     (EVAL single-arg env)]
...
(define (eval_ast [ast : Mal] [env : mal-env]) : Mal
  ...
  (match ast
    ...
    [(? symbol? sym)
     (env-get env sym)]
    [(mal-list _ _ xs)
     (mal-list #f #f (map eval-with-env xs))]
    [(mal-vector _ _ v)
     (mal-vector #f #f (vector->immutable-vector (vector-map eval-with-env v)))]
    [(mal-hash _ _ m)
     (mal-hash #f #f
...
(define (mal-quasiquote [ast : Mal]) : Mal
  (match ast
    [(mal-vector _ _ v)
     (mal-list #f #f (list 'vec (mal-qq-list (vector->list v))))]
    [(mal-hash _ _ h)
     (mal-list #f #f (list 'quote ast))]
    [(? symbol? s)
     (mal-list #f #f (list 'quote s))]
    [(mal-list _ _ (list 'unquote x))
     x]
    [(mal-list _ _ '())
     (mal-list #f #f '())]
    [(mal-list _ _ xs)
...
(define (mal-qq-list [list-vals : (Listof Mal)]) : Mal
  (match list-vals
    [(list (mal-list _ _ (list 'splice-unquote val)) rest ...)
     (mal-list #f #f (list 'concat val (mal-qq-list rest)))]
    [(list val rest ...)
     (mal-list #f #f (list 'cons (mal-quasiquote val) (mal-qq-list rest)))]
    ['()
     (mal-list #f #f '())]
    [_
...
(define (get-macro [ast : Mal] [env : mal-env]) : (U #f (Pair (-> (Listof Mal) Mal) (Listof Mal)))
  (match ast
    [(mal-list _ _ (list (? symbol? s) args ...))
...
(module+ test
  ...
  (let ([e (env-new '() #f)]
        [m (lambda ([args : (Listof Mal)]) (mal-list #f #f args))])
    (env-set! e 's1 (mal-macro m))
    (env-set! e 's2 2)
    (check-equal? (get-macro (mal-list #f #f (list 's1 2 3)) e) (cons m '(2 3)))
    (check-equal? (get-macro (mal-list #f #f (list 's1)) e) (cons m '()))
    (check-false (get-macro (mal-list #f #f (list 's2)) e))
    (check-false (get-macro 3 e))
    (check-false (get-macro (mal-list #f #f '()) e))))
