...
use jw_rust_mal::types::{
    err, into_mal_closure, into_mal_hashmap, into_mal_seq, is_falsy, sym, Env, Mal, MalError,
    MalResult,
};
    ...
    loop {
        ...
        if let Mal::Seq(true, xs, _meta) = ast.clone() {
            match xs.as_slice() {
                ...
                // try* special form
                [Mal::Symbol(n), a, Mal::Seq(true, xs, _meta)] if n == "try*" => {
                    if let [Mal::Symbol(m), b, c] = xs.as_slice() {
                        if m == "catch*" {
                            match EVAL(a.clone(), env.clone()) {
                                Err(MalError::Exception(e)) => {
                                    let params: Vec<Mal> = vec![b.clone()];
                                    let exprs: Vec<Mal> = vec![e.clone()];
                                    ast = c.clone();
                                    env = env::new_binds(Some(&env), &params, &exprs)?;
                                }
                                Err(MalError::Msg(msg)) => {
                                    let params: Vec<Mal> = vec![b.clone()];
                                    let exprs: Vec<Mal> = vec![Mal::String(msg)];
                                    ast = c.clone();
                                    env = env::new_binds(Some(&env), &params, &exprs)?;
                                }
                            ...
                            return err("Missing catch* symbol in try* special form");
                        }
                    } else {
                        return err("Missing catch* clause in try* special form");
                    }
                }
